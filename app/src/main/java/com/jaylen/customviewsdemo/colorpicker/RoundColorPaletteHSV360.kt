 ...
 * 圆形取色盘
 */
class RoundColorPaletteHSV360 constructor(
    context: Context,
    attrs: AttributeSet?,
    defStyleAttr: Int
    ...
    init {
        ...
        strokeColor =
            typedArray.getColor(R.styleable.RoundColorPaletteHSV360_strokeColor, Color.YELLOW)
        isOutOfBounds =
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (width > 0) {
            val size = min(width,height) - paddingStart - paddingEnd
            if (size > 0) {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (w > 0 && h > 0){
            radius = min(w,h).coerceAtMost(h) / 2f
